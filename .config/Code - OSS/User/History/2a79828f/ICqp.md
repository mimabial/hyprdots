---
DENv exposes `xdg_config/denv/config.toml` file for users to modify. This lets users have the ability to interact the scripts without using command arguments.

Users are encouraged to use an editor that support schema validation to ensure the configuration file is valid.
```toml
"$schema" = "https://raw.githubusercontent.com/DENv-Project/DENv/refs/heads/master/Configs/.local/share/denv/schema/config.toml.json"
```
---
### [battery.notify]
...
| bar | Bar. | denv-shell waybar --watch |
