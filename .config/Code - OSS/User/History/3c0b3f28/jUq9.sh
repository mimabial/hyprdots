...
rofiStyleDir="${SHARE_DIR}/denv/rofi/themes"
rofiAssetDir="${SHARE_DIR}/denv/rofi/assets"
...
if [ -n "${RofiSel}" ]; then
    ...
    notify-send -a "DENv Alert" -r 2 -t 2200 -i "${rofiAssetDir}/${RofiSel}.png" " style ${RofiSel} applied..."
fi
if [ -n "$ROFI_LAUNCH_STYLE" ]; then
    notify-send -a "DENv Alert" -r 3 -u critical "Style: '$ROFI_LAUNCH_STYLE' is explicitly set, remove it in ~/.config/denv/config.toml for changes to take effect."
fi
