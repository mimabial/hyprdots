...
landing_dir="${XDG_CACHE_HOME:-$HOME/.cache}/denv/landing/fonts"
...
download_and_extract() {
    ...
    find "$temp_dir" -type f | while read -r file; do
        ...
        *.tar.xz)
            ...
            ;;
        *.ttf | *.otf)
            mkdir -p "${font_dir}/denv"
            mv "$file" "${font_dir}/denv/$name.ttf"
            echo "[font] $name installed successfully. Please restart hyprlock to apply changes."
            notify-send -i "preferences-desktop-font" "DENv font" "${name} Installed successfully"
            return 0
            ;;
        *)
        ...
        if ! cp -rn "${temp_dir}/${name}" "$font_dir"; then
            ...
            notify-send -i "preferences-desktop-font" "DENv font" "Failed to extract $file"
            return 1
        fi
        notify-send -i "preferences-desktop-font" "DENv font" "${name} Installed successfully"
    done
