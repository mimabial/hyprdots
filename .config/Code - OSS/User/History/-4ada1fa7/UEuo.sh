...
cacheDir="${HYDE_CACHE_HOME:-"${XDG_CACHE_HOME}/denv"}"
...
fn_profile() {
    ...
    if [ -f "$HOME/.face.icon" ]; then
        ...
        cp "$XDG_DATA_HOME/icons/Wallbash-Icon/denv.png" "${profilePath}.png"
    fi
    return 0
}
...
fn_mpris() {
    ...
    if [[ "${player_status}" == "Playing" ]]; then
        ...
        if [ -f "$HOME/.face.icon" ]; then
            ...
            if ! cmp -s "$XDG_DATA_HOME/icons/Wallbash-Icon/denv.png" "${THUMB}.png"; then
                cp "$XDG_DATA_HOME/icons/Wallbash-Icon/denv.png" "${THUMB}.png"
                pkill -USR2 hyprlock >/dev/null 2>&1 # updates the mpris thumbnail
            fi
        fi
        exit 1
...
generate_conf() {
    ...
    local denv_hyprlock_conf=${SHARE_DIR:-$XDG_DATA_HOME}/denv/hyprlock.conf
...
# If a persistent layout path is declared in \$XDG_CONFIG_HOME/hypr/denv.conf,
...
# Alternatively, you can statically source the layout in '~/.config/hypr/denv.conf'.
...
# DENv also provides custom variables that extend the functionality of Hyprlock.
