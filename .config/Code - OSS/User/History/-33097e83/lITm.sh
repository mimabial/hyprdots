...
fn_wallbash() {
    ...
    WALLBASH_SCRIPTS="${template%%denv/wallbash*}denv/wallbash/scripts"
    if [[ "${template}" == *.theme ]]; then
        ...
        if [[ -n "${dcolTemplate}" ]]; then
            ...
            WALLBASH_SCRIPTS="${dcolTemplate%%denv/wallbash*}denv/wallbash/scripts"
    ...
    # shellcheck disable=SC2154
    [ -f "$DENV_STATE_HOME/state" ] && source "$DENV_STATE_HOME/state"
    # shellcheck disable=SC1091
    ...
    export WALLBASH_SCRIPTS confDir denvConfDir cacheDir thmbDir dcolDir iconsDir themesDir fontsDir wallbashDirs enableWallDcol DENV_THEME_DIR DENV_THEME GTK_ICON GTK_THEME CURSOR_THEME
...
source "${dcol_file}"
# shellcheck disable=SC2154
if [ -f "${DENV_THEME_DIR}/theme.dcol" ] && [ "${enableWallDcol}" -eq 0 ]; then
    # shellcheck disable=SC1091
    source "${DENV_THEME_DIR}/theme.dcol"
    print_log -sec "wallbash" -stat "override" "dominant colors from ${DENV_THEME} theme"
    print_log -sec "wallbash" -stat " NOTE" "Remove \"${DENV_THEME_DIR}/theme.dcol\" to use wallpaper dominant colors"
fi
...
if [ "${enableWallDcol}" -eq 0 ] && [[ "${reload_flag}" -eq 1 ]]; then
    ...
    print_log -sec "wallbash" -stat "apply ${dcol_mode} colors" "${DENV_THEME} theme"
    mapfile -d '' -t deployList < <(find "${DENV_THEME_DIR}" -type f -name "*.theme" -print0)

    while read -r pKey; do
        fKey="$(find "${DENV_THEME_DIR}" -type f -name "$(basename "${pKey%.dcol}.theme")")"
