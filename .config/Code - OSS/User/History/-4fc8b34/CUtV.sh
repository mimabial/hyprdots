...
notes: 
       --backend <backend> is also use to cache wallpapers/background images e.g. hyprlock
           ...
           ~/.cache/denv/wallpapers/hyprlock.png
...
Wall_Json() {
    setIndex=0
    [ ! -d "${DENV_THEME_DIR}" ] && echo "ERROR: \"${DENV_THEME_DIR}\" does not exist" && exit 0
    wallPathArray=("${DENV_THEME_DIR}")
    ...
    jq -n --argjson wallList "$wallListJson" --argjson wallHash "$wallHashJson" --arg cacheHome "${DENV_CACHE_HOME:-$HOME/.cache/denv}" '
...
main() {
    ...
    if [ "$set_as_global" == "true" ]; then
        wallSet="${DENV_THEME_DIR}/wall.set"
        wallCur="${DENV_CACHE_HOME}/wall.set"
        wallSqr="${DENV_CACHE_HOME}/wall.sqre"
        wallTmb="${DENV_CACHE_HOME}/wall.thmb"
        wallBlr="${DENV_CACHE_HOME}/wall.blur"
        wallQad="${DENV_CACHE_HOME}/wall.quad"
        wallDcl="${DENV_CACHE_HOME}/wall.dcol"
    elif [ -n "${wallpaper_backend}" ]; then
        mkdir -p "${DENV_CACHE_HOME}/wallpapers"
        wallCur="${DENV_CACHE_HOME}/wallpapers/${wallpaper_backend}.png"
        wallSet="${DENV_THEME_DIR}/wall.${wallpaper_backend}.png"
    else
        wallSet="${DENV_THEME_DIR}/wall.set"
    fi
    ...
    if [ -f "${scrDir}/wallpaper.${wallpaper_backend}.sh" ] && [ -n "${wallpaper_backend}" ]; then
        ...
        if command -v "wallpaper.${wallpaper_backend}.sh" >/dev/null; then
            ...
            print_log -warn "wallpaper" "Created: $DENV_CACHE_HOME/wallpapers/${wallpaper_backend}.png instead"
        fi
    fi
    ...
    if [ "${wallpaper_setter_flag}" == "select" ]; then
        if [ -e "$(readlink -f "${wallSet}")" ]; then
            if [ "${set_as_global}" == "true" ]; then
                notify-send -a "DENv Alert" -i "${selected_thumbnail}" "${selected_wallpaper}"
            else
                notify-send -a "DENv Alert" -i "${selected_thumbnail}" "${selected_wallpaper} set for ${wallpaper_backend}"
            fi
        else
            notify-send -a "DENv Alert" "Wallpaper not found"
        fi
    fi
}
