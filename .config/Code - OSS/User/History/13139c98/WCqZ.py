...
def get_venv_path():
    ...
    venv_path = os.path.join(xdg_base_dirs.xdg_state_home(), "denv", "pip_env")
    if not os.path.exists(venv_path):
        venv_path = os.path.expanduser("~/.local/state/denv/pip_env")
...
def create_venv(venv_path, requirements_file=None):
    ...
    if not os.path.exists(os.path.join(venv_path, "bin", "pip")):
        ...
        notify.send("DENv PIP", "✅ Virtual environment created successfully")
    else:
        pass
...
def v_import(module_name):
    ...
    try:
        ...
        notify.send("DENv PIP", f"Installing {module_name} module...")
        ...
        try:
            module = importlib.import_module(module_name)
            notify.send("DENv PIP", f"Successfully installed {module_name}.")
            return module
...
def main(args):
    parser = argparse.ArgumentParser(description="Python environment manager for DENv")
    ...
    requirements_file = os.path.join(
        os.path.expanduser("~/.local/lib/denv/pyutils"), "requirements.txt"
    )
...
def denv(args):
    """Python environment manager for DENv.
