...
monitor_height=$((monitor_info[1] * 95 / monitor_scale))
  ...
  BG=$HOME/.local/share/denv/rofi/assets/steamdeck_holographic.png
  BGfx=$HOME/.cache/denv/landing/steamdeck_holographic_${monitor_width}x${monitor_height}.png
...
fn_steam() {
  ...
  if [ -z "${ManifestList}" ]; then
    notify-send -a "DENv Alert" "Cannot Fetch Steam Games!" && exit 1
  fi
  ...
  if [ -n "$RofiSel" ]; then 
    ...
    notify-send -a "DENv Alert" -i "$headerImage" "Launching ${RofiSel}..."
  fi
}
