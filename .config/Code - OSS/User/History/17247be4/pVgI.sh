...
image_dirs=()
denv_distro_logo=${iconDir}/Wallbash-Icon/distro/$LOGO
...
logo) # eats around 13 ms
  random() {
    (
      ...
      if [ -n "${DENV_THEME}" ] && [ -d "${confDir}/denv/themes/${DENV_THEME}/logo" ]; then
        image_dirs+=("${confDir}/denv/themes/${DENV_THEME}/logo")
      fi
      # [ -d "$DENV_CACHE_HOME" ] && image_dirs+=("$DENV_CACHE_HOME")
      [ -f "$denv_distro_logo" ] && echo "${denv_distro_logo}"
      image_dirs+=("$DENV_CACHE_HOME/wall.quad")
      image_dirs+=("$DENV_CACHE_HOME/wall.sqre")
      [ -f "$HOME/.face.icon" ] && image_dirs+=("$HOME/.face.icon")
      [ -f "$denv_distro_logo" ] && image_dirs+=("$denv_distro_logo")
      image_dirs+=("${confDir}/fastfetch/logo")
      image_dirs+=("${iconDir}/Wallbash-Icon/fastfetch/")
      if [ -n "${DENV_THEME}" ] && [ -d "${confDir}/denv/themes/${DENV_THEME}/logo" ]; then
        image_dirs+=("${confDir}/denv/themes/${DENV_THEME}/logo")
...
options:
  ...
  --theme   Display a logo inside the denv theme directory
