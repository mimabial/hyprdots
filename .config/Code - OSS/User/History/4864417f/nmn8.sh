...
mpris_fallback_image() {
  ...
  if [ -f "$HOME/.face.icon" ]; then
    if ! cmp -s "$HOME/.face.icon" "$target_file"; then
      cp -f "$HOME/.face.icon" "$target_file"
      pkill -USR2 hyprlock >/dev/null 2>&1
    fi
  else
    if ! cmp -s "$XDG_DATA_HOME/icons/Wallbash-Icon/denv.png" "$target_file"; then
      cp "$XDG_DATA_HOME/icons/Wallbash-Icon/denv.png" "$target_file"
      pkill -USR2 hyprlock >/dev/null 2>&1
    fi
  fi
}
...
mpris_thumb() { # Generate thumbnail for mpris
  ...
  # Check if artUrl is empty or invalid
  if [[ -z "$artUrl" || "$artUrl" == "{{mpris:artUrl}}" ]]; then
    mpris_fallback_image
    return 0
  fi
  ...
  if [[ "$artUrl" =~ ^file:// ]]; then
    local_file="${artUrl#file://}"
    if [ -f "$local_file" ]; then
      [ "${artUrl}" == "$(cat "${THUMB}".lnk 2>/dev/null)" ] && [ -f "${THUMB}".png ] && return 0
      echo "${artUrl}" >"${THUMB}".lnk
      if magick "$local_file" -quality 50 "${THUMB}.png" 2>/dev/null; then
        pkill -USR2 hyprlock >/dev/null 2>&1
        return 0
      fi
    fi
  ...
  if curl -Lso "${THUMB}.art" --connect-timeout 10 --max-time 30 "$artUrl" 2>/dev/null; then
    if magick "${THUMB}.art" -quality 50 "${THUMB}.png" 2>/dev/null; then
      pkill -USR2 hyprlock >/dev/null 2>&1
    else
      # Try direct copy if magick fails
      if ! cp "${THUMB}.art" "${THUMB}.png" 2>/dev/null; then
        mpris_fallback_image
      fi
      pkill -USR2 hyprlock >/dev/null 2>&1
    fi
...
#*│ \$XDG_CONFIG_HOME/hypr/denv.conf,                                          │
...
#*┌──────────────────────────────────────────────────────────────────────────┐
#*│    All boilerplate configurations are handled by DENv                  │
#*└──────────────────────────────────────────────────────────────────────────┘
...
#┌────────────────────────────────────────────────────────────────────────────┐
#│ DENv also provides custom variables to extend hyprlock's functionality.  │
#│                                                                            │
#│   \$BACKGROUND_PATH                                                        │
...
#│   - if available, otherwise, it will show the DENv logo.                   │
#│                                                                            │
#│   \$PROFILE_IMAGE                                                          │
...
#│   - DENv will run 'font.sh resolve' to install the font for you.           │
