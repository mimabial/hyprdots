...
if ! source "$(command -v denv-shell)"; then
    echo "[wf-recorder] code :: Error: denv-shell not found."
    echo "[wf-recorder] code :: Is DENv installed?"
    exit 1
fi
...
if ! command -v "$RECORDER" &>/dev/null; then
    notify-send -a "DENv Alert" "No screen recorder found. Try installing wl-screenrec or wf-recorder."
...
Usage: 'denv-shell screenrecord' [option]
...
Example:
        'denv-shell screenrecord' --start -- --audio --codec libx264
...
handle_recording() {
    ...
    if [[ -z "$GEOM" ]]; then
        "$LIB_DIR/denv/grimblast" save active "$tmp_thumbnail"
    else
        grim -g "$GEOM" "$tmp_thumbnail"
    fi

    "${RECORDER}" "${parameters[@]}" -f "${save_file_path}"
    notify-send -a "DENv Alert" "${RECORDER}: Recording saved at ${save_file_path}" -i "${tmp_thumbnail}"
}
...
while [[ $# -gt 0 ]]; do
    ...
    --quit)
        killall "${RECORDER}"
        notify-send -a "DENv Alert" "Recording stopped"
        exit 0
        ;;
    --help)
        USAGE
