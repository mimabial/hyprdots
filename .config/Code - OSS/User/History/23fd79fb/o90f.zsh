...
function _dedup_zsh_plugins {
    ...
    # Load Plugins
    denv_plugins=(git zsh-256color zsh-autosuggestions zsh-syntax-highlighting)
    plugins+=("${plugins[@]}" "${denv_plugins[@]}")
    # Deduplicate plugins
    plugins=("${plugins[@]}")
    plugins=($(printf "%s\n" "${plugins[@]}" | sort -u))
...
function _load_deferred_plugin_system_by_denv() {

    # Exit early if HYDE_ZSH_DEFER is not set to 1
    if [[ "${HYDE_ZSH_DEFER}" != "1" ]]; then
        unset -f _load_deferred_plugin_system_by_denv
        return
    fi
...
HYDE_ZSH_DEFER="1"      #Unset this variable in $ZDOTDIR/user.zsh to disable DENv's deferred Zsh loading.
HYDE_ZSH_PROMPT="1"     #Unset this variable in $ZDOTDIR/user.zsh to disable DENv's prompt customization.
HYDE_ZSH_NO_PLUGINS="1" #Unset this variable in $ZDOTDIR/user.zsh to disable DENv's deferred Zsh loading.
...
if [[ -f $HOME/.denv.zshrc ]]; then
    source $HOME/.denv.zshrc # for backward compatibility
...
if [[ ${HYDE_ZSH_NO_PLUGINS} == "1" ]]; then
    ...
    if [[ "$HYDE_ZSH_OMZ_DEFER" == "1" ]]; then
        _load_deferred_plugin_system_by_denv
