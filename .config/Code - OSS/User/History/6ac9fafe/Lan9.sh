...
[ -d "${DENV_THEME_DIR}" ] && cacheIn="${DENV_THEME_DIR}" || exit 1
...
fn_wallcache() {
    ...
    if [ "${is_video}" -eq 1 ]; then
        ...
        then
            ...
            notify-send -a "DENv wallpaper" "Extracting thumbnail from video wallpaper..."
...
# Function to cache any links that are denv related
fn_envar_cache() {
    if command -v rofi &>/dev/null; then
        if [[ ! "$XDG_DATA_DIRS" =~ share/denv ]]; then
            mkdir -p "$XDG_DATA_HOME/rofi/themes"
            ln -snf "$XDG_DATA_HOME/denv/rofi/themes"/* "$XDG_DATA_HOME/rofi/themes/"
        fi
    fi
}
...
while getopts "w:t:f" option; do
    ...
    t) # generate cache for input theme
        cacheIn="$(dirname "${DENV_THEME_DIR}")/${OPTARG}"
        if [ ! -d "${cacheIn}" ]; then
    ...
    f) # full cache rebuild
        cacheIn="$(dirname "${DENV_THEME_DIR}")"
        mode="_force"
        ;;
    *) # invalid option
        echo "... invalid option ..."
