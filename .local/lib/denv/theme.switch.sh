...
[[ -d "$DENV_CACHE_HOME/wallpapers/" ]] && find "$DENV_CACHE_HOME/wallpapers" -name "*.png" -exec sh -c '
    for file; do
...
if [ "$quiet" = true ]; then
  "${scrDir}/wallpaper.sh" -s "$(readlink "${DENV_THEME_DIR}/wall.set")" --global >/dev/null 2>&1
else
  "${scrDir}/wallpaper.sh" -s "$(readlink "${DENV_THEME_DIR}/wall.set")" --global
fi
