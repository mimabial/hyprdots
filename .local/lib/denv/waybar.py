...
STATE_FILE = Path(os.path.join(str(xdg_state_home()), "denv", "staterc"))
DENV_CONFIG = Path(os.path.join(str(xdg_state_home()), "denv", "config"))
...
def get_config_value(key, default=None):
    ...
    if DENV_CONFIG.exists():
        with open(DENV_CONFIG, "r") as file:
            for line in file:
...
def write_style_file(style_filepath, source_filepath):
    ...
    wallbash_gtk_css_file = os.path.join(
        str(xdg_cache_home()), "denv", "wallbash", "gtk.css"
    )
...
def run_waybar_command(command):
    ...
    log_dir = os.path.join(str(xdg_runtime_dir()), "denv")
...
def manage_waybar_lock(action="toggle"):
    ...
    """
    lock_file = os.path.join(str(xdg_runtime_dir()), "denv", "waybar_hide.lock")
...
def main():
    ...
    source_env_file(os.path.join(str(xdg_runtime_dir()), "denv", "environment"))
    source_env_file(os.path.join(str(xdg_state_home()), "denv", "config"))
...
def get_value_from_hypr_theme(variable_name):
    ...
    theme_dir = os.path.join(str(xdg_config_home()), "denv", "themes", theme_name)
    ...
    if not border_radius:
        ...
        if theme_name:
            theme_dir = os.path.join(
                str(xdg_config_home()), "denv", "themes", theme_name
            )
            logger.debug(f"Looking for theme directory at: {theme_dir}")
...
def update_style(style_path):
    ...
    if not os.path.exists(theme_style_filepath):
        logger.error(
            f"Missing '{theme_style_filepath}', Please run 'denv-shell reload' to generate it."
        )
